    ...
    return (
        <>
            <nav className='bg-custom-287581 text-white nav-features'>
              ...
              {userRole === "admin" && <NavLink to="/calendrier" className={({isActive})=>(isActive?"active":undefined)}>Calendrier</NavLink>}    
              {userRole === "admin" && <NavLink to="/profil-des-joueurs" className={({isActive})=>(isActive?"active":undefined)}>Profil des joueurs</NavLink>}    
              {userRole === "admin" && <NavLink to="/tableau-de-bord" className={({isActive})=>(isActive?"active":undefined)}>Tableau de bord</NavLink>}    
            </nav>
        </>
    )
}